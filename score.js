const GRADE_CHART = {
  ...
  const LIKELIHOOD_INDICATOR_CHART = {
	'A': 'LOW',
	'B': 'MEDIUM',
	'C': 'MEDIUM',
	'D': 'MEDIUM',
  ...
  const GRADES = new Set(Object.values(GRADE_CHART));
  const SCORE_TABLE = {
	  // contribute.json
	  ...
	  'csp-implemented-with-unsafe-inline-in-style-src-only': {
		  'description': ('Content Security Policy (CSP) implemented with unsafe sources inside style-src, This includes \'unsafe-inline\', data: or overly broad sources such as https:.'),
		  'modifier': 0,
	  },
	  'csp-implemented-with-insecure-scheme-in-passive-content-only': {
		  'description': ('Content Security Policy (CSP) implemented, but secure site allows images or media to be loaded over HTTP'),
		  'modifier': -10,
	  },
	  ...
	  'csp-implemented-with-insecure-scheme': {
		  'description': ('Content Security Policy (CSP) implemented, but secure site allows resources to be loaded over HTTP'),
		  'modifier': -20,
	  },
	  'csp-header-invalid': {
	  ...
	  'cookies-secure-with-httponly-sessions-and-samesite': {
		  'description': ('All cookies use the Secure flag, session cookies use the HttpOnly flag, and cross-origin restrictions are in place via the SameSite flag'),
		  'modifier': 5,
	  ...
	  'cross-origin-resource-sharing-implemented-with-public-access': {
		  'description': ('Public content is visible via cross-origin resource sharing (CORS) Access-Control-Allow-Origin header'),
		  'modifier': 0,
	  },
	  ...
	  'sri-implemented-but-external-scripts-not-loaded-securely': {
		  'description': ('Subresource Integrity (SRI) implemented, but external scripts are loaded over HTTP or use protocol-relative URLs via src="//..."'),
		  'modifier': -20,
	  },
